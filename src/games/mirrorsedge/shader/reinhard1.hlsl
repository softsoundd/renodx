#ifndef SRC_SHADERS_REINHARD_HLSL_1
#define SRC_SHADERS_REINHARD_HLSL_1
...
float ComputeReinhardScale(float peak = 1.f, float minimum = 0.f, float gray_in = 0.18f, float gray_out = 0.18f) {
  ...
  // Avoid literal Inf/NaN on older shader models.
  float abs_den = abs(den);
  float den_sign = (den < 0.f) ? -1.f : 1.f;
  den = (abs_den < 1e-6f) ? den_sign * 1e-6f : den;
